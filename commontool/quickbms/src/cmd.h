...
int calcc(char *input) {
    ...
    // the following allocation is lame and consumes lot of memory, I will fix it in the next versions
    static char *buff = NULL;
    buff = realloc(buff, (strlen(input) * (2 /*"0x"*/ + (sizeof(int) * 2))) + 1);
    if(!buff) return 0;
    strcpy(buff, input);
        ...
        *data = 0;
        ...
        for(;;) {
            ...
            left  = calcc_get_num(++l);
            right = calcc_get_num(++p);
    ...
    if(len2) {
        switch(op) {
            ...
            case 'p': { // *printf-like
                ...
                /*
                // unfortunately C doesn't allow to pass values of different sizes
                float   float_tmp[NUM(0)];
                double  double_tmp[NUM(0)];
                for(i = 0; i < NUM(0); i++) {
                    if((args_format[i] == 'f') || (args_format[i] == 'g')) {
                        len2 = VAR32(3 + i);
                        memcpy(&float_tmp[i], &len2, sizeof(float));
                        double_tmp[i] = float_tmp[i];
                    }
                }
                #define STRING_PRINTF(N) \
                    (myisdigitstr(VAR(3+N)) ? (((args_format[N] == 'f') || (args_format[N] == 'g')) ? double_tmp[N] : (void *)VAR32(3+N)) : VAR(3+N))
                */
...
int CMD_CallDLL_func(int cmd, u8 *input, u8 *output) {
    ...
    if(input || output) {
        for(i = 0; i < argc; i++) { // wow, looks chaotic?
            n = CMD.var[4 + i];
            if(!strcmp(get_varname(n), "#INPUT#"))  {
                args[i] = input;
            } else if(!strcmp(get_varname(n), "#OUTPUT#")) {
                args[i] = output;
            }
        }
    }
...
#define CALLDLL_DO_INT3 \
        if(g_int3) { \
            __asm__ __volatile__ ("movl %0, %%eax" :: "g"(dllname)  :        "ecx", "edx", "ebx", "esi"); \
            __asm__ __volatile__ ("movl %0, %%ecx" :: "g"(hlib)     : "eax",        "edx", "ebx", "esi"); \
            __asm__ __volatile__ ("movl %0, %%edx" :: "g"(funcname) : "eax", "ecx",        "ebx", "esi"); \
            __asm__ __volatile__ ("movl %0, %%ebx" :: "g"(funcaddr) : "eax", "ecx", "edx",        "esi"); \
            __asm__ __volatile__ ("movl %0, %%ebp" :: "g"(argc)     : "eax", "ecx", "edx", "ebx"       ); \
