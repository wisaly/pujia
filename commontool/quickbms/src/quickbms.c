...
i32 main(i32 argc, char *argv[]) {
    ...
    if(!winapi_missing()) {
        ...
        //if(_AddVectoredContinueHandler) _AddVectoredContinueHandler(1, VectoredHandler);
    ...
    int check_if_running_from_doubleclick(void) {
        ...
        if(g_osver.dwMajorVersion > 4) {
            ...
            if(GetWindowLong(GetForegroundWindow(), GWL_WNDPROC)) {
                return 0;//modified by kris always return 0//return 1;
            }
            return 0;
        }
    ...
    if(check_if_running_from_doubleclick() == 1) g_is_gui = 1;
    ...
    for(i = 1; i < argc; i++) {
        ...
        switch(argv[i][1]) {
            case '-':
            case '?':
            case 'h': myhelp(argv[0]);  myexit(QUICKBMS_ERROR_ARGUMENTS);   break;
            ...
            case '9': XDBG_ALLOC_ACTIVE     = !XDBG_ALLOC_ACTIVE;           break;  // xdbg_toggle() is not ready yet
