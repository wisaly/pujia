...
int read_mem(HANDLE hp, void *addr, void *buff, int size) {
    ...
    #endif
    rp = ReadProcessMemory(hp, addr, buff, size, &ret);
    ...
    if(GetThreadContext(ht, &ctx)) {
        ...
        if(dbg->u.Exception.ExceptionRecord.ExceptionCode == STATUS_BREAKPOINT) {
            ...
            if(i < process_int3s) {
                ...
                if(hp) {
                    write_mem(hp, (void *)ctx.Eip, &(process_int3[i].byte), 1);
...
u8 *debug_get_string(HANDLE hp, void *addr, int unicode, int write4) {
    ...
    if(!addr) return("");
    if(write4) {
        if(read_mem(hp, addr, (void *)&t, sizeof(void *)) < 0) return("");
        s = (u8 *)t;
    } else {
        s = addr;
    }
    ...
    if(s) {
        ...
        for(i = 0; i < (sizeof(tmp) - 1); i++) {
            if(read_mem(hp, s + i, &c, 1) < 0) break;
...
quick_thread(process_debugger, process_file_t *proc) {
    ...
    while(proc->debug) {
        ...
        if(dbg->dwDebugEventCode == EXCEPTION_DEBUG_EVENT) {
            ...
            if(
                ...
                //if(!dbg->u.Exception.dwFirstChance) {
                    ...
                    fprintf(stderr,
                        "\n"
                        "- Press RETURN when you want to try to continue the execution of the\n"
        ...
        // Thread create
            ...
            if(verbose) printf("DEBUG thread        %08x %p %p\n",
        ...
        // Process create
            ...
            if(verbose) printf("DEBUG process       %08x %08x %08x %p %08x %08x %p %p %s\n",
        ...
        // Thread exit
            ...
            if(verbose) printf("DEBUG thread exit   %08x\n",
        ...
        // Process exit
            ...
            if(verbose) printf("DEBUG exit          %08x\n",
            ...
            if(verbose) printf("DEBUG dll load      %08x %p %s\n",
            ...
            if(verbose) printf("DEBUG dll unload    %p\n",
            ...
            p = debug_get_string(proc->hp, dbg->u.DebugString.lpDebugStringData, dbg->u.DebugString.fUnicode, 1);
            if(verbose) printf("DEBUG debug string  %p\n",
                p);

        } else if(dbg->dwDebugEventCode == RIP_EVENT) {
            add_var(0, "dwDebugEventCode", "RIP_EVENT", 0, -1);
            if(verbose) printf("DEBUG rip           %08x %08x\n",
