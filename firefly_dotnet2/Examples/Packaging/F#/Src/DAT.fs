...
type DAT =
    ...
    new(sp : ZeroPositionStreamPasser) as this = { inherit PackageDiscrete(sp) } then
        let s = sp.GetStream()
    ...
    static member Open (path : String) : PackageBase =
        new DAT(
            ZeroPositionStreamPasser (
                try
                    new StreamEx(path, FileMode.Open, FileAccess.ReadWrite)
                with _ ->
                    new StreamEx(path, FileMode.Open, FileAccess.Read)
            )
        ) :> PackageBase
    ...
    override this.FileAddressInPhysicalFileDB
        with get (file : FileDB) : int64 =
            this.BaseStream.Position <- 16L + 44L * int64 this.IndexOfFile.[file] + 32L
            int64 (this.BaseStream.ReadInt32())
        and set (file : FileDB) (value : int64) =
            this.BaseStream.Position <- 16L + 44L * int64 this.IndexOfFile.[file] + 32L
            this.BaseStream.WriteInt32(int32 value)

    //读取文件在索引中的长度信息，所有索引中的长度信息应该在这里更新
    override this.FileLengthInPhysicalFileDB
        with get (file : FileDB) : int64 =
            this.BaseStream.Position <- 16L + 44L * int64 this.IndexOfFile.[file] + 36L
            int64 (this.BaseStream.ReadInt32())
        and set (file : FileDB) (value : int64) =
            this.BaseStream.Position <- 16L + 44L * int64 this.IndexOfFile.[file] + 36L
            this.BaseStream.WriteInt32(int32 value)
