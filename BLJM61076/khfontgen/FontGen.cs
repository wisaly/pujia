...
namespace khfontgen
{
    ...
    class FontGen
    {
            ...
            Font font,bool drawGrid,out Bitmap bmp,string savepath)
        {
            ...
            {
                for (int j = 0; j < 4 && i < chars.Count; j++, i++)
                {
                    CharUnit cu = new CharUnit();
                    cu.charVal = chars[i];
                    cu.x = (i / 4) % (totWidth / charWidth) * charWidth;
                    cu.y = (i / 4) / (totWidth / charWidth) * charHeight;
                    cu.actrualWidth = 0;
                    cu.cr = fontColor[j];
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                foreach (CharUnit cu in charUnits)
                {
                    ...
                    TextRenderer.DrawText(g, c, font, charLeftTop, cu.cr, TextFormatFlags.NoPadding);
